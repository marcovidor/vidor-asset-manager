...
export async function GET(req: NextRequest) {
  ...
  const asset_id = sanitize(new URL(req.url).searchParams.get('asset_id') || '')
...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await auth.supabase.from('checkouts').insert({
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await auth.supabase
    .from('checkouts').update({ checked_in_at: body.checked_in_at })
    .eq('id', sanitize(body.id)).select().single()
